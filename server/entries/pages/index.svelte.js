import { n as noop, d as safe_not_equal, f as now, l as loop, c as create_ssr_component, a as subscribe, e as escape, v as validate_component } from "../../chunks/index-22591d9c.js";
...
const Counter = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<div class="${"counter svelte-ppn7h8"}"><button aria-label="${"Decrease the counter by one"}" class="${"svelte-ppn7h8"}"><svg aria-hidden="${"true"}" viewBox="${"0 0 1 1"}" class="${"svelte-ppn7h8"}"><path d="${"M0,0.5 L1,0.5"}" class="${"svelte-ppn7h8"}"></path></svg></button>
	...
	<div class="${"counter-viewport svelte-ppn7h8"}"><div class="${"counter-digits svelte-ppn7h8"}" style="${"transform: translate(0, " + escape(100 * offset) + "%)"}"><strong class="${"hidden svelte-ppn7h8"}" aria-hidden="${"true"}">${escape(Math.floor($displayed_count + 1))}</strong>
			<strong class="${"svelte-ppn7h8"}">${escape(Math.floor($displayed_count))}</strong></div></div>
